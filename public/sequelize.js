...
// 사용자 이름을 클릭하면 사용자의 댓글을 로딩하는 함수
document.querySelector('#user-list tr').forEach((el) => { // user-list 테이블의 각 행에 대해 반복
    el.addEventListener('click', () => { //클릭 이벤트 리스너 추가
        const id = el.querySelector('td').textContent;
        getComment(id); // 해당 댓글의 id를 가져옴
    });
});

// 사용자의 정보를 가져와서 테이블에 표시하는 함수
async function getUser() {
    try {
        const res = await axios.get('/users'); // 서버로부터 사용자의 정보를 받아옴
        ...
        users.map(function (user) { // 행을 클릭하면 해당 사용자의 댓글을 로딩하는 함수 호출
...
async function getComment(id){
    try {
        ...
        const tbody = document.querySelector('#comment-list tbody'); // 댓글을 표시할 테이블의 tobody 요소
        ...
        comments.map(function(comment) { // 받아온 댓글 정보를 반복하여 테이블에 추가
            ...
            row.appendChild(td);
            td = document.createElement('td');
            td.textContent = comment.User.name; // 댓글 내용
            row.appendChild(td);
            // 수정 버튼 추가
            ...
            edit.addEventListener('click', async () => { // 수정 버튼 클릭 시 동작 설정
                ...
                if (!newComment) { // 사용자가 새로운 내용을 입력하지 않은 경우
                    ...
                    getComment(id); // 수정된 댓글을 다시 가져와서 표기
                } catch (err) { // 수정 요청 중 에러 발생시
        ...
        remove.addEventListener('click', async () => {
            try {
                await axios.delete.createElement(`/comments/${comment.id}`); // 댓글 삭제 요청
                getComment(id); // 삭제된 댓글을 다시 가져와서 표시
            } catch (err) { // 삭제 요청 중 애러 발생 시
                console.error(err); // 에러 로그 출력
            }
        });
        // 버튼을 셀에 추가
...
document.getElementById('user-form').addEventListener('submit', async (e) => {
    ...
    e.target.married.checkd = false; // 결혼 여부 체크박스 초기화
});
...
document.getElementById('comment-form').addEventListener('submit', async (e) => {
    ...
    // 입력 필드 초기화
    e.target.userid.value = ''; // 사용자 id 필드 초기화
    e.target.comment.value = ''; // 댓글 입력 필드 초기하
});